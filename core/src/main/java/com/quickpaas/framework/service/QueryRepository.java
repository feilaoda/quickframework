...
@Slf4j
@Component
public class QueryRepository<D extends BaseDomain> {
    ...
    @Autowired
    public QueryRepository(ServiceRegistry serviceRegistry, ClassCache classCache) {
    ...
    private Map<Long, BaseDomain<?>> findTargetEntitiesByIds(JoinRelation join, QueryField field, String columnName, List<Long> ids) {
        ...
        List<BaseDomain> entities = quickService.selectEntitiesByIds(field, quickService.getColumnName(join.getTarget(), columnName), ids);
...
//    }
    ...
    public BaseQuickService<E, D> findService(Class<?> clz) {
